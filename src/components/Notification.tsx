...
const Notification: React.FC<NotificationProps> = ({ title, message }) => {
  ...
  return (
    <div className={dynamicClassNames(show ? "right-6" : "-right-96", "fixed top-6 w-full max-w-96 transition-[right] ease-out duration-200 z-30")}>
      <div className="p-4 bg-white rounded-lg shadow-lg ring-1 ring-black ring-opacity-5">
        <div className="flex items-start">
          <div className="flex-shrink-0">
            <CheckCircleIcon className={`w-6 h-6 stroke-green-400`} aria-hidden="true" />
          </div>
          ...
          <div className="flex-shrink-0 flex ml-4">
            <button onClick={handleClose}>
              <XMarkIcon className="w-5 h-5 stroke-gray-400" aria-hidden="true" />
            </button>
          </div>
        </div>
      </div>
