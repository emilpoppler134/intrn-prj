  ...
  return (
    <>
      <header className="bg-white">
        <nav className="mx-auto flex max-w-5xl items-center justify-between p-4 lg:px-8" aria-label="Global">
        ...
        <Dialog as="div" className="lg:hidden" open={mobileMenuOpen} onClose={setMobileMenuOpen}>
          ...
          <Dialog.Panel className="fixed inset-y-0 right-0 z-10 w-full overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
            <div className="flex items-center justify-between">
              <Link to="/" className="-m-1.5 p-1.5">
                <span className="sr-only">Netlight</span>
                <img className="h-8 w-auto" src={`${API_ADDRESS}/images/logo_icon.png`} alt="Logo" />
              </Link>
                    ...
                    {({ open }: any) => (
                      <>
                        <Disclosure.Button className="flex w-full items-center justify-between rounded-lg py-2 pl-3 pr-3.5 text-base font-semibold leading-7 text-gray-900 hover:bg-gray-50">
                          Product
                          <ChevronDownIcon className={dynamicClassNames(open ? "rotate-180" : "", "h-5 w-5 flex-none")} aria-hidden="true" />
                        </Disclosure.Button>
                      </>
                    )}
                  </Disclosure>
      ...
      <main className="mx-auto flex flex-col max-w-5xl pt-4 lg:px-8">{children}</main>
