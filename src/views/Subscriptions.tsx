...
export default function Subscriptions() {
  ...
  const form = useForm([[
    { key: "productId", validation: null }
  ]]);

  const [error, setError] = useState<string | null>(null);
  const [products, setProducts] = useState<Array<Product> | null | undefined>(undefined);

  useEffect(() => {
    callAPI<Array<Product>>("/products/list")
      .then(response => {
        ...
        const validDataResponse = response as ValidDataResponse & { data: Array<Product> };
  ...
  const handleCreatePaymentIntent = async ({ productId }: FormValues) => {
    const response = await callAPI<PaymentIntent>("/subscriptions/create-payment-intent", { id: productId });
    ...
    if (response.status === ResponseStatus.ERROR) {
      switch (response.error) {
        case ErrorType.ALREADY_EXISTING: {
          setError("You already have an active subscription. Cancel it to start a new one.");
          return;
        }
    ...
    const validDataResponse = response as ValidDataResponse & { data: PaymentIntent };

    navigate("/subscriptions/payment", {state: {paymentIntent: validDataResponse.data}});
  }
    ...
    >
      <div className="mx-auto max-w-2xl">
        {products.map(product => (
          <div key={product.id} className="-mt-2 mx-auto p-2 lg:mt-0 lg:w-full lg:max-w-md lg:flex-shrink-0">
            <div
